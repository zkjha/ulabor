            ...
            createElement:function (dataList) {
                ...
                for(var i = 0,l=list.length;i<l;i++){
                    ...
                    switch (data.strReviewResult){//状态
                        case '0':
                            str +="<td class=' groupType_td ' ><span class=' notInGroup groupType i18n'  data-title='Auditing'>审核中</span></td>";//是否加入
                            break;
                        case '1':
                            str +="<td class=' groupType_td ' ><span class=' groupType joinGroup i18n'  data-title='agree'>已通过</span></td>";//是否加入
                            break;
                        case '-1':
                            str +="<td class=' groupType_td ' ><span class=' groupType refuseGroup i18n'  data-title='refuse'>已拒绝</span></td>";//是否加入
                            break;
                        default:
                            str +="<td class=' groupType_td ' ><span class=' notInGroup groupType i18n'  data-title='Auditing'>审核中</span></td>";
                    }
                    //获取用户信息用来处理不同操作

                    //如果还没有加入，则只能处理是否加入
                    if(data.strReviewResult == 0){
                        str +="<td><span class='btn i18n action_span agree_group in_group ' data-title='actionAgree' data-id='"+dataId+"'>同意</span>" +
                            "<span class='btn action_span i18n refuse_group in_group ' data-title='actionRefuse' data-id='"+dataId+"'>拒绝</span>"+
                            "<span class='btn action_span i18n refuse_group detail ' data-title='actionDetail' data-id='"+dataId+"'>详情</span>"
                    }else{
                        // //查看群成员
                        ...
                        // }
                        str+="<td> <span class='btn action_span i18n refuse_group detail ' data-title='actionDetail' data-id='"+dataId+"'>详情</span>"
