...
var doCombine = function(callback) {
  ...
  var name = path.slice((distfolder + jsDirName).length + 1, -3);
  var combineOptions = {
    'baseUrl' : distfolder + jsDirName,
    'name' : name,
    'out' : path,
...
var walkJavascripts = function(path) {
  ...
  filesList.forEach(function(currentFile) {
    ...
    if (fs.statSync(tPath).isDirectory()) {
      ...
      if (ext == 'js') {
        if (tPath.indexOf(distfolder + jsDirName+'/page') != -1) {
...
exports.run = function(callback) {
  ...
  try {
    utils.log('正在压缩通用脚本...');
    walkJavascripts(distfolder + jsDirName);
    utils.log('压缩处理完毕！');
    utils.log('正在合并脚本...');
    utils.log('请稍后...');
    doCombine(function() {
